...
describe 'navigate' do
  ...
  it 'displays a link to the genre page' do
    ...
    expect(page).to have_link("Genre", href: genre_path(@genre)) #@genre.name, href: genre_path(@genre))
  end

  it 'displays a link to the artist page' do
    visit song_path(@song.id)

    expect(page).to have_link("Artist", href: artist_path(@artist)) #@artist.name
  end
...
describe 'form' do
  ...
  it 'shows a new form that submits content and redirects and prints out params' do
    ...
    fill_in 'song_artist_id', with: @artist.id
    fill_in 'song_genre_id', with: @genre.id
